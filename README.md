...
└── data/
    ├── base_ehcvm_education_niveauvie_benin2021.csv  # Base de données enrichie (77 variables)
    └── carte_benin_fond.jpg                          # Fond de carte du Bénin (cartographie)
```
...
1. **🏠 Accueil** : Indicateurs clés (KPIs), contexte du mémoire, **cartographie interactive du Bénin** (bulles proportionnelles : niveau de vie et scolarisation par département) et graphiques d'évolution conjointe du niveau de vie selon le profil éducatif.
...
3. **📉 Analyse Économétrique** : Tests de spécification complets (Multicolinéarité VIF, Hétéroscédasticité de Breusch-Pagan, Normalité Jarque-Bera, Ramsey RESET), régressions progressives MCO robustes (HC3), **traitement de l'endogénéité par doubles moindres carrés (2SLS) avec Test de Hausman**, et **analyse de l'hétérogénéité spatiale** (MCO par département + Test de Chow).
4. **🤖 Machine Learning** : Comparaison rigoureuse de 7 algorithmes de régression (Ridge, Lasso, Elastic Net, Random Forest, Gradient Boosting, SVM, XGBoost) avec validation croisée, **optimisation des hyperparamètres (GridSearchCV)**, scores et importance des variables.
